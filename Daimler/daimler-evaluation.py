...
if __name__ == "__main__":
	...
	for Threshold in ThreshRange:
		...
		NumofImages = 0.0
		FNtot = 0.0
		for files in os.scandir(detection_results):
			if files.is_file() and files.name.endswith('.csv'):
				resfile = os.path.join(detection_results, files.name)
				#print(resfile)
				annfile = os.path.join(annotation_path, os.path.splitext(files.name)[0] +".pgm"+".txt")
				#print(annfile)
