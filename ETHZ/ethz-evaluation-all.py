...
if __name__ == "__main__":
	...
	for Threshold in ThreshRange:
		...
		for folder in next(os.walk(detection_results))[1]: # return folder
			res_path = os.path.join(detection_results, folder, 'detection_csv')
			for files in os.scandir(res_path):
				if files.is_file() and files.name.endswith('.csv'):
					resfile = os.path.join(res_path, files.name)
					#print(resfile)
					annfile = os.path.join(annotation_path, folder, os.path.splitext(files.name)[0] +".png"+".txt")
					#print(annfile)
